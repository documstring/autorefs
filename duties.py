...
PY_SRC_LIST = ("src/docums_autorefs", "tests", "duties.py")
...
def check_docs(ctx):
    ...
    ctx.run("docums build -s", title="Building documentation", pty=PTY, nofail=nofail, quiet=nofail)
...
def docs(ctx):
    ...
    Arguments:
        ctx: The context instance (passed automatically).
    """
    ctx.run("docums build", title="Building documentation")
...
def docs_serve(ctx, host="127.0.0.1", port=8000):
    ...
    """
    ctx.run(f"docums serve -a {host}:{port}", title="Serving documentation", capture=False)
...
def docs_deploy(ctx):
    ...
    Arguments:
        ctx: The context instance (passed automatically).
    """
    ctx.run("docums gh-deploy", title="Deploying documentation")
